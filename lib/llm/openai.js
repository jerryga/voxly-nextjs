...
export async function answerProjectQuestionOpenAI({
  prompt,
  model,
} = {}) {
  if (!prompt || !prompt.trim()) {
    throw new Error("answerProjectQuestion requires a prompt");
  }
      ...
      messages: [
        {
          role: "system",
          content:
            "You answer project-level or workspace-level questions from provided transcript excerpts only. Return strict JSON only.",
        },
        { role: "user", content: prompt },
      ],
    });
