...
export function normalizeTemplate(template) {
  const normalized = String(template || "")
    .trim()
    .toLowerCase();
  const mapped = templateAliases[normalized] || normalized;
  return allowedTemplates.includes(mapped) ? mapped : "default";
}

export function buildPrompt(
  transcript,
  { template = "default", customInstructions } = {},
) {
...
function getCustomInstructions(customInstructions) {
