...
function getProviderOrder(preferred) {
  ...
  console.log("Provider order:", base);
  return base;
}
...
export async function formatTranscript(
  ...
  return runWithProviders(order, async (prov) => {
    if (prov === "gemini") {
      return formatTranscriptGemini(rawTranscript, readableTranscript, {
        model,
        template,
      });
    }
    if (prov === "openai") {
      return formatTranscriptOpenAI(rawTranscript, readableTranscript, {
        model,
        template,
      });
    }
...
export async function answerProjectQuestion({
  ...
  return runWithProviders(order, async (prov) => {
    if (prov === "gemini")
      return answerProjectQuestionGemini({ prompt, model });
    if (prov === "openai")
      return answerProjectQuestionOpenAI({ prompt, model });
    throw new Error(`Unsupported provider ${prov}`);
  });
}
